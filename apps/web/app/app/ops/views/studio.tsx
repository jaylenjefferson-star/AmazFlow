...
// The builder lives outside ops/ because it is shared with the pre-existing console shell and
// carries its own validation. Studio renders it rather than keeping a second copy.
...
export function StudioView({ workflowId }: { workflowId?: string }) {
  ...
  // The control plane rejects a malformed workflow with one flat string. The builder already
  // knows exactly what is wrong, so refuse to submit rather than explain a rejection.
  const blockers = draft ? workflowBlockingProblems(draft) : [];
        ...
        actions={
          <>
            ...
            {draft && (
              <Btn
                variant="primary"
                disabled={!dirty || blockers.length > 0 || actions.busy === `saveWorkflow_${draft.id}`}
                onClick={async () => {
