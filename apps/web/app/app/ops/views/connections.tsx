...
export function AgentsView() {
  ...
  // The control plane computes connectionStatus itself (agentSnapshot) and that is the value the
  // rest of the platform gates on, so prefer it. The heartbeat-age fallback only covers records
  // written before that field existed.
  const online = (agent: { lastSeenAt: string | null; status: string; connectionStatus?: string }) => {
    if (agent.status === "revoked") return false;
    if (agent.connectionStatus) return agent.connectionStatus === "connected";
    return Boolean(agent.lastSeenAt) && Date.now() - new Date(agent.lastSeenAt as string).getTime() < 10 * 60_000;
  };

  /** Permissions a Desktop App needs but has not been granted. Explains a stalled desktop step. */
  const missingPermissions = (agent: { agentType?: string; permissions?: { accessibility?: boolean; screenRecording?: boolean } }) => {
    if (agent.agentType !== "DESKTOP_AGENT" || !agent.permissions) return [];
    const missing: string[] = [];
    if (agent.permissions.accessibility === false) missing.push("Accessibility");
    if (agent.permissions.screenRecording === false) missing.push("Screen Recording");
    return missing;
  };
  ...
  return (
    <>
      <PageHead
        title="Agents"
        sub="The two surfaces AmazFlow executes on: the Chrome Extension for websites, and the Desktop App for installed Mac applications. A workflow step is only offered to a surface whose agent advertised that action."
        actions={
          ...
          columns={[
            {
              ...
              nowrap: true,
              render: (agent) => {
                if (agent.status === "revoked") return <Pill tone="muted">Revoked</Pill>;
                const missing = missingPermissions(agent);
                if (online(agent) && missing.length > 0) {
                  return <Pill tone="waiting" title={`Needs ${missing.join(" and ")}`}>Needs permission</Pill>;
                }
                if (online(agent)) return <Pill tone="good" dot>Connected</Pill>;
                if (agent.lastSeenAt) return <Pill tone="waiting">{CONNECTION_STATE_LABEL.offline}</Pill>;
                return <Pill tone="muted">Never connected</Pill>;
              },
            },
            {
              key: "surface",
              ...
              render: (agent) =>
                agent.agentType ? (
                  <Pill tone="muted">{AGENT_TYPE_LABEL[agent.agentType] ?? agent.agentType}</Pill>
                ) : (
                  <span className="ops-muted">Chrome Extension</span>
                ),
            },
            {
              key: "name",
              ...
              render: (agent) => (
                <CellStack
                  top={agent.name}
                  bottom={[
                    agent.version ? `v${agent.version}` : "version not reported",
                    agent.platform || null,
                    agent.capabilities?.length ? `${agent.capabilities.length} actions` : null,
                  ].filter(Boolean).join(" · ")}
                />
              ),
            },
            ...
            },
            {
              ...
              header: "Scoped to",
              render: (agent) =>
                agent.agentType === "DESKTOP_AGENT" ? (
                  <span className="ops-muted">apps named by each step</span>
