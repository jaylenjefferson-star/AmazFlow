...
class AmazFlowAgentCoreStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    // Customer-managed secrets (task 20.2/20.3): scoped by name prefix, since each secret's ARN does
    // not exist until CreateSecret runs. GetSecretValue is deliberately absent -- no route ever reads
    // a customer secret's value back (requirement 20.8).
    control.addToRolePolicy(new iam.PolicyStatement({ actions: ["secretsmanager:CreateSecret", "secretsmanager:PutSecretValue", "secretsmanager:DeleteSecret"], resources: [this.formatArn({ service: "secretsmanager", resource: "secret", resourceName: "amazflow/customer-secret/*", arnFormat: cdk.ArnFormat.COLON_RESOURCE_NAME })] }));
